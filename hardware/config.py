...
#
# retain=True 로 발행한다: 서버가 나중에 떠도 브로커가 마지막 상태를 바로 물려준다.
# offline은 **우리가 보내는 게 아니라 브로커가 LWT로 대신 보낸다** — 전원이 나가거나
# 랜선이 빠지면 정상 종료 코드가 돌 기회조차 없기 때문이다. 그게 LWT의 존재 이유다.
...
# ─── UART (STM32) ───
UART_PORT    = "/dev/ttyAMA0"   # 주원이 카메라 코드와 동일 포트 (ttyAMA10→ttyAMA0, 2026-07-15 주원 반영)
UART_BAUD    = 115200
...
#   전제: 바닥 마커를 **전부 같은 방향으로** 깔아야 한다. 한 장이라도 돌아가면 그 노드만 틀어진다.
#
