...
# ── 명령 입력용 큐 ────────────────────────────────────────────
# 표준 입력(터미널)을 별도 스레드에서 읽어 큐에 넣는다.
# 이렇게 하면 메인 비전 루프가 input() 때문에 멈추지 않는다.
cmd_queue = queue.Queue()


def _stdin_reader(q):
    """별도 스레드에서 계속 입력을 받아 큐에 넣기만 한다."""
    while True:
        try:
            line = input()
        except EOFError:
...
def live_aruco_detection(calibration_data):
    ...
    command = 0x00
    waiting_for_cmd = False   # EVT_DONE을 받고 명령을 기다리는 중인지 표시

    # 입력 전용 스레드 시작 (daemon이라 메인 종료 시 함께 종료됨)
        ...
        while True:
            ...
            if uart.in_waiting > 0:
                ...
                if rx_data[0] == EVT_DONE:
                    print("EVT_DONE - 다음 명령 입력 (숫자 입력 후 Enter): ", flush=True)
                    waiting_for_cmd = True   # 여기서 input()으로 막지 않는다
                    print(hex(rx_data[0]))
            ...
            # ── 명령 입력을 논블로킹으로 처리 ──────────────────
            # 큐에 들어온 입력이 있으면 꺼내서 command에 반영한다.
            # input()과 달리 큐가 비어 있으면 그냥 통과하므로 화면이 끊기지 않는다.
            if waiting_for_cmd and not cmd_queue.empty():
                cmd_str = cmd_queue.get_nowait()
                try:
                    command = int(cmd_str)
                    waiting_for_cmd = False
                    print(f"명령 설정: {command}")
                except ValueError:
                    print(f"잘못된 입력: {cmd_str!r} — 숫자를 입력하세요")
            ...
            # 이미지 왜곡 보정 (미리 만든 맵으로 remap → undistort보다 빠름)
            frame_undistorted = cv2.remap(frame_rgb, map1, map2, cv2.INTER_LINEAR)
