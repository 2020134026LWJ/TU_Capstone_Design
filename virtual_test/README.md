...
## 모드 1.5 — 수동 조종 (★트윈이 제대로 도는지 눈으로 보는 용도)

**서버는 진짜 그대로 돌고(주문·경로계획·충돌회피 전부), 사람이 'AGV의 몸' 역할만 한다.**
...
# 터미널 3 — 수동 조종 AGV (★ 반드시 진짜 터미널에서. stdin 필요)
python3 -m virtual_test.bench_camera.run_bench 1 --no-camera --manual 9

# 터미널 4 — 주문
mosquitto_pub -h localhost -t warehouse/order/start -m '{"사용자ID":1,"주문번호":101,"작업대":2}'
```

```
[Enter]  서버 명령을 '완료했다'고 보고 (forward→도착 마커 / turn·lift→cmd_ack)
[숫자]   임의 마커 강제 발행 (예: 37 → 서버 가드가 막는지 확인)
[s]      상태 보기      [q] 종료
```

**왜 이 모드가 트윈 검증에 제일 좋은가**: **한 칸 소요시간을 사람이 정한다.**
빨리 치면 빠른 로봇, 10초 뜸 들이면 느린 로봇. 트윈의 페이싱·홀드·스냅이 그 변화를
따라오는지 눈으로 본다. `--auto-walk`는 속도가 일정해서 이 '흔들기'를 못 한다.

**볼 것** (Isaac 로그):
- `실측 1칸 3.4초 → 평균 3.3초 (n=5)` — 당신이 기다린 시간을 트윈이 학습하는 중
- `(먼저 도착해 0.6초 대기)` — 정상. 수정 65는 **일부러 빠르게** 편향해 먼저 닿게 한다
- `⚠ 스냅 0.45m` — 트윈이 늦어서 끌려갔다는 뜻 (Enter를 너무 빨리 쳤을 때)
- `⚠ 대기 12초` — 너무 오래 뜸 들였다 (실물이라면 "끼었다"는 신호)

**일부러 망가뜨려 보기**: Enter를 아주 빨리 연타하면 트윈이 못 따라가 스냅이 나고,
아주 천천히 치면 트윈이 먼저 가서 기다린다. **두 실패 모드를 직접 만들어볼 수 있다.**
