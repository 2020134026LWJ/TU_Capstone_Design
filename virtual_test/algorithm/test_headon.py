...
맵(8×6, 노드 0~47): row2=8~15(W2=8), row1=0~7 / row6=40~47 = bypass. 모든 열에 세로 간선.
"""
...
def _setup_headon(handler):
    """row2에서 정면 충돌: AGV-1@11 동향(→12), AGV-2@12 서향(→11). 둘 다 blocked.
    ...
    r1.current_node = 11
    r1.heading = 90               # 동 → 다음 노드 12
    r1.heading_initialized = True
    r1.planned_path = [11, 12, 13, 14, 22]
    r1.command_queue = ["forward"]
    r1.carrying_shelf = 22
    sm.mark_shelf_picked_up(22, 1)

    r2 = handler.robot_manager.get_robot(2)
    r2.current_node = 12
    r2.heading = 270              # 서 → 다음 노드 11
    r2.heading_initialized = True
    r2.planned_path = [12, 11, 10, 9, 8]
    r2.command_queue = ["forward"]
    r2.carrying_shelf = 29
    sm.mark_shelf_picked_up(29, 2)
...
def test_staging_robot_included_in_deadlock_cycle(handler):
    ...
    재현: AGV-1@11(→12 막힘, W2 회랑 점유) ↔ AGV-2@12(W2 줄서기로 12에 주차).
    """
    r1 = handler.robot_manager.get_robot(1)
    r1.current_node = 11
    r1.heading = 90                    # 동 → 다음 12
    r1.heading_initialized = True
    r1.planned_path = [11, 12, 10, 9, 8]
    r1.command_queue = ["forward"]
    ...
    # AGV-2를 W2 줄서기로 등록(12에 주차), W2 회랑은 AGV-1이 점유 중
    handler.staging_manager.add_staged_agv(ws_node=8, rid=2, staging_node=12)
    handler.reservation.reserve_indefinite(1, 8)
...
def test_staging_deadlock_resolves(handler):
    ...
    """
    # AGV-1: W2 회랑 점유 중이며 15 쪽으로 빠져나가는 길에 12를 지나야 함 (12에 AGV-2 주차)
    r1 = handler.robot_manager.get_robot(1)
    r1.current_node = 11
    r1.heading = 90                    # 동 → 다음 12
    r1.heading_initialized = True
    r1.planned_path = [11, 12, 13, 14, 15]
    r1.command_queue = ["forward"]

    # AGV-2: W2 줄서기로 12에 주차 (빈 큐), W2 회랑은 AGV-1이 점유
    r2 = handler.robot_manager.get_robot(2)
    r2.current_node = 12
    r2.heading_initialized = True
    r2.command_queue = []
    ...
    # 양보자 = 움직이는 AGV-1. 막던 staging 로봇 노드(12)를 피하고 목적지(15)는 유지.
    assert r1.planned_path, "양보자 새 경로가 나와야 함"
    assert r1.planned_path[-1] == 15, "목적지(15) 유지"
    assert 12 not in r1.planned_path, \
        f"막던 staging 로봇 노드(12)를 피해야 함: {r1.planned_path}"


def test_no_false_positive_when_not_mutual(handler):
    """한쪽만 막힌(상대가 마주보지 않음) 경우는 사이클 미형성 → None."""
    r1, r2 = _setup_headon(handler)
    # AGV-2를 옆 행으로 치워 상호성 깨기 (AGV-1의 다음 12가 비게 됨)
    r2.current_node = 20
    r2.heading = 0
    r2.planned_path = [20, 28]
    r2.command_queue = ["forward"]
    cycle = handler._detect_deadlock_cycle()
...
def test_resolve_breaks_cycle(handler):
    ...
    # 양보자 = 낮은 rid = AGV-1. contested(12)를 피하고 목적지(22) 유지.
    assert r1.planned_path, "양보자 새 경로가 나와야 함"
    assert r1.planned_path[-1] == 22, "양보자 목적지(22)는 유지"
    assert 12 not in r1.planned_path, \
        f"양보자가 contested 노드(12)를 피해야 함: {r1.planned_path}"
    # 승자(AGV-2) 경로는 안 건드림
    assert r2.planned_path == [12, 11, 10, 9, 8], "승자 경로는 그대로"


def test_resolve_uses_bypass_row(handler):
    """운반 중 우회 경로는 bypass 행(row1 0~7 또는 row6 40~47)을 경유한다."""
    _setup_headon(handler)
    cycle = handler._detect_deadlock_cycle()
    assert handler._resolve_deadlock(cycle) is True

    r1 = handler.robot_manager.get_robot(1)
    bypass = set(range(0, 8)) | set(range(40, 48))
...
def test_try_dispatch_all_resolves_freeze(handler):
    ...
    # 양보자(AGV-1)가 contested(12)를 피한 새 경로를 받음
    assert 12 not in r1.planned_path, \
...
def test_detect_4robot_rotational_cycle(handler):
    ...
    10-11-19-18 사각형을 시계 방향으로 도는 4대가 각자 앞을 막아 회전 교착.
      10→11→19→18→10 (각 간선 존재)
    """
    cfg = [(1, 10, 90),   # @10 동 →11
           (2, 11, 180),  # @11 남 →19
           (3, 19, 270),  # @19 서 →18
...
def test_chain_is_not_cycle(handler):
    ...
    _inject_robot(handler, 3, 10)
    # AGV-3@10→11(AGV-1), AGV-1@11→12(AGV-2), AGV-2@12→20(빈 노드) = 체인
    r1 = handler.robot_manager.get_robot(1)
    r1.current_node, r1.heading, r1.heading_initialized = 11, 90, True
    r1.command_queue = ["forward"]
    r2 = handler.robot_manager.get_robot(2)
    r2.current_node, r2.heading, r2.heading_initialized = 12, 0, True  # →20 빈 노드
    r2.command_queue = ["forward"]
    r3 = handler.robot_manager.get_robot(3)
    r3.current_node, r3.heading, r3.heading_initialized = 10, 90, True  # →11
