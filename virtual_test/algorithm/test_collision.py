...
def test_blocks_when_target_occupied(handler, mock_mqtt):
    ...
    a = _ready_robot(handler, rid=1, node=10, heading=90)
    _ready_robot(handler, rid=2, node=11, heading=0)  # B가 A의 next 점유
    ...
    assert handler.command_queues[1].peek_expected_node() != 11, \
...
def test_blocks_when_target_reserved_by_other(handler, mock_mqtt):
    ...
    handler.command_queues[2].in_flight = CommandEntry(cmd="forward", target_node=11)
    ...
    assert handler.command_queues[2].peek_expected_node() == 11
...
def test_resumes_after_blocker_moves(handler, mock_mqtt):
    ...
    a = _ready_robot(handler, rid=1, node=10, heading=90)
    b = _ready_robot(handler, rid=2, node=11, heading=0)
    a.command_queue = ["forward"]
...
def test_marker_arrival_clears_reservation(handler, mock_mqtt):
    ...
    handler.handle_message(json.dumps({
        "type": "marker_report", "rid": 1, "marker_id": 11, "heading": 90,
    }))

    assert handler.command_queues[1].peek_expected_node() != 11, \
        "마커 도착 시 큐 ack로 in_flight 비워짐"
    assert a.current_node == 11
