...
def test_offline_robot_is_never_dispatched(handler):
    ...
    _prime(handler, 1, node=8, online=False)
    _prime(handler, 2, node=32, online=True)
...
def test_no_robot_available_when_all_offline(handler):
    ...
    _prime(handler, 1, node=8, online=False)
    _prime(handler, 2, node=32, online=False)
...
def test_lwt_offline_stops_further_dispatch(handler):
    ...
    _prime(handler, 1, node=8, online=True)
    _prime(handler, 2, node=32, online=False)
...
def test_marker_report_marks_robot_online(handler):
    ...
    robot = _prime(handler, 1, node=8, online=False)
...
def test_never_connected_robot_does_not_block_path(handler):
    ...
    _prime(handler, 1, node=1, online=True)
    _prime(handler, 2, node=2, online=False)   # 1→3 최단 경로 한가운데. 하지만 없는 로봇이다.

    handler._plan_and_publish_move(1, 1, 3)

    path = handler.robot_manager.get_robot(1).planned_path
    assert 2 in path, (
        f"존재하지도 않는 AGV-2 때문에 노드 2를 우회했다 (경로 {path}). "
        "안 켠 로봇이 길을 막으면 안 된다."
    )
...
def test_disconnected_robot_still_blocks_path(handler):
    ...
    """
    _prime(handler, 1, node=1, online=True)
    robot2 = _prime(handler, 2, node=2, online=True)   # 켜진 적 있다 = 바닥에 있다
    ...
    assert 2 not in path, (
        f"연결 끊긴 AGV-2가 서 있는 노드 2를 통과하는 경로를 냈다 (경로 {path}). "
