...
> ## ⚠️ 번호 체계 변경 (2026-07-14) — 이 문서를 읽기 전에
>
> **노드 번호와 ArUco 마커 ID가 `1~48` → `0~47`로 바뀌었다** (0-based). 맵의 모양·좌표·연결은
> 그대로고 **번호만 1씩 내려갔다.** 노드 ID == 마커 ID 항등은 계속 유지된다.
>
> | | 옛 (1~48) | 지금 (0~47) |
> |---|---|---|
> | W1 (작업대1) | 33 (gw 25 / stg 41 / trg 34) | **32** (gw 24 / stg 40 / trg **33**) |
> | W2 (작업대2) | 9 (gw 17 / stg 1 / trg 10) | **8** (gw 16 / stg **0** / trg 9) |
> | 선반 | 19 20 27 28 / 22 23 30 31 | **18 19 26 27 / 21 22 29 30** |
> | AGV 홈 | AGV-1=9, AGV-2=33 | AGV-1=**8**, AGV-2=**32** |
> | 입고 | 48 | **47** |
>
> **아래 "수정 이력"(수정 1~75)의 노드 번호는 전부 옛 1~48 기준이다** — 그때 그렇게 관찰된
> 기록이라 손대지 않았다. **현재 번호 = 옛 번호 − 1**로 읽으면 된다.
>
> ⚠️ **노드 0이 유효 노드가 됐다** → 코드에서 `if not node:` 같은 truthiness 판단 금지 (`is None` 사용).
> 실제로 이것 때문에 W2 회랑이 통째로 등록 안 되는 버그가 났다 (staging=0이 falsy).

> **왜 이렇게 고치는가 / 근본 해결 방식**(개발자 아닌 사람도 읽을 수 있는 평문)은
> [`../설계_근본해결_노트.md`](../설계_근본해결_노트.md) 참고 (저장소 밖 상위 폴더 `Projects/`).
> 이 문서(FLOWCHART)는 기술 상세·수정 이력.
>
...
### 수정 62: 마커 인접성 검사 — 오검출 순간이동 차단 (2026-07-12)

> **쉽게**: 실물 카메라 벤치 중 **카메라 앞에 아무것도 없는데** 마커 37 → 3 → 4가 차례로 검출됐다. 서버는 이걸 믿고 로봇을 맵 반대편으로 **순간이동**시켰고, heading 장부가 실제와 어긋나 `turn_left`를 냈는데 차가 엉뚱한 방향을 봤다. Isaac 트윈이 `heading 방향 노드 없음`으로 신고해서 발각됐다 — **트윈이 서버의 잘못된 믿음을 잡아낸 첫 사례.**
>
> **수정 59가 왜 못 막았나**: 그 필터는 "맵에 있는 번호냐"만 본다. 그런데 `DICT_4X4_250`의 오검출은 ID 0~249에 고르게 흩어지고 우리 유효 노드는 1~48이라, **5번 중 1번(≈19%)은 유효 노드로 위장해 그냥 통과한다.** 145는 걸렀지만 37은 못 걸렀다.

- **가드** (`_marker_mixin._handle_marker_report`, 수정 59 필터 바로 뒤): 로봇은 한 칸씩 굴러가지 순간이동하지 않는다 → 다음에 올 수 있는 마커는 **현재 노드**(아직 안 떠남) 아니면 **이웃 노드**(도착)뿐. 그 외는 무시(`non_adjacent_marker`). **통과 가능한 문이 48개 → 3~4개로 줄고**, 뚫려도 피해가 한 칸에 그친다.
- **추가**: `PathPlanner.neighbors(node)` (`graph`는 이미 인접 리스트라 조회만 노출).
- **전제**: AGV는 **설정된 홈 노드에서 시작해야 한다**(AGV-1=9, AGV-2=33). "첫 마커는 무조건 믿는다"는 부트스트랩 예외는 **일부러 안 넣었다** — 이번 사고가 정확히 그 시나리오(서버 켜자마자 유령 마커 37)라 예외를 두면 버그가 그대로 남는다.
- **마커를 놓친 경우에도 버리는 게 맞다**: 위치는 '모르는 값'보다 '직전 값'이 안전 (수정 59와 같은 논리).
- **근본 처방(미적용)**: `DICT_4X4_250` → `DICT_6X6_50`. ID 48개만 필요한데 250개짜리를 쓸 이유가 없다. **마커 재인쇄 필요** → 바닥에 마커 깔 때 같이.
- **검증**: pytest **89 passed**(신규 6개 — 37→3→4 시퀀스 재현 포함). 실서버에 마커 주입 → `불가능한 마커 37 무시 (현재 9, 이웃 [10,1,17])` 확인, 정상 마커 17은 수용.

**수정 파일**: `server/core/_marker_mixin.py`(가드), `server/planning/path_planner.py`(`neighbors`), `virtual_test/algorithm/test_marker_adjacency.py`(신규)

> ⚠️ **[정정] "ArUco 오검출"이라는 원인 진단은 틀렸다** (같은 날 밝혀짐 — 수정 64 참고).
> 실제 원인은 **마커 시트에 여러 개가 같이 인쇄돼 있는 것**이었다. `make_marker_sheet.py`는
> A4 한 장에 15~20개를 격자로 찍는다(4열×5행이면 1페이지에 노드 1~20) → **9번을 보여주려고
> 시트를 들면 옆칸 10번, 윗줄 3·4번이 함께 시야에 든다.** `camera.py`는 여러 개가 잡히면
> **첫 번째를 반환**한다.
> **오검출 실측**: 정지 상태 2000프레임 → **0건**. 진짜 마커를 들고 300프레임 → **ID 오독 0건**,
> yaw 프레임간 변화 중앙값 0.20°. ArUco 자체는 멀쩡했다.
> **수정 62는 그대로 유효하다** — 원인이 무엇이든 "물리적으로 불가능한 위치 보고"를 막는
> 가드는 필요하고, 실제로 3·4번을 막았다. 다만 **10번은 9의 이웃이라 못 막았고**, 그래서
> 수정 64가 필요했다.

---

### 수정 63: MQTT client_id 충돌 → 무한 재연결 루프 (2026-07-12)

> **쉽게**: 트윈(Isaac)과 실물/벤치(라파)를 같이 띄우면 **AGV-1 브릿지만** MQTT 재연결을 무한 반복했다(10초에 4회, 로그에 `MQTT connected` 73회). AGV-2는 멀쩡했다 — **AGV-2에는 짝이 없기 때문**이다.

- **원인**: `client_id = f"bridge_{rid}_{int(time.time())}"` — **초 단위 타임스탬프로 유일성**을 만들었다. Isaac은 씬 로딩에 십수 초가 걸린 뒤 브릿지를 만드는데, 그 사이 벤치를 띄우면 **같은 초에 겹쳐 client_id가 똑같아진다.** MQTT는 같은 client_id를 허용하지 않아 브로커가 먼저 붙은 쪽을 끊고, 끊긴 쪽이 재연결하며 상대를 다시 끊는다 → **서로 무한히 밀어냄. 명령이 유실된다.**
- **수정**: `pid + uuid`로 확실히 분리 + **역할별 접두사**(실물/벤치 `bridge_`, 트윈 `twin_`). 같은 초에 떠도 절대 겹치지 않고 브로커에서 누가 누군지 구분도 된다.
- **검증**: Isaac 73회 → **2회**(AGV-1·AGV-2 브릿지 각 1회 = 정상), 벤치 72회 → **1회**.
- **왜 위험한가**: 타이밍에 달려 있어 **재현이 안 된다.** 시연 당일에만 터지면 원인 추적이 거의 불가능한 종류.

**수정 파일**: `isaac_simulation/bridge_isaac.py`, `hardware/bridge_rpi.py`

---

### 수정 64: forward 목표와 다른 마커 거부 — 한 칸 순간이동 차단 (2026-07-12)

> **쉽게**: 서버가 **"17번으로 forward 해라"라고 명령해둔 상태**에서 마커 10이 들어왔다. 서버는 불일치를 **감지하고도 WARN만 찍고 그대로 믿어** 로봇을 10번으로 옮기고 거기서 경로를 다시 짰다. AGV가 한 칸 순간이동한 셈.
>
> **마커 10은 어디서 왔나**: 오검출이 아니다. **마커 시트 한 장에 15~20개가 격자로 인쇄**돼 있어(9번 옆칸이 10번), 9번을 보여주려고 시트를 드니 10번이 같이 잡혔다. `camera.py`는 여러 개면 첫 번째를 반환한다.

- **수정 62(인접성)로는 못 막는다**: 10은 9의 **정당한 이웃**이라 통과한다. 수정 62가 3·4번은 막았지만 10번은 통과시켰다 — "뚫려도 피해가 한 칸"이라던 그 경우가 실제로 났다.
- **가드**: `in_flight.cmd == "forward"`이면 **도착할 수 있는 곳은 목표 노드 하나뿐**이다. 그 외 마커는 무시(`marker_target_mismatch`). 통과 가능한 문이 **이웃 3~4개 → 목표 1개**로 줄어든다. 기존 코드는 이미 불일치를 **감지**하고 있었다(`WARN marker mismatch`) — 믿지만 않으면 됐다.
- **시트와 무관하게 필요**: 실물에서도 마커를 놓치거나 잘못 읽으면 같은 사고가 난다.
- **운용 권고**: 벤치 테스트 시 **시트를 낱장 카드로 잘라 쓸 것.** 한 번에 한 마커만 보이게.
- **검증**: pytest **91 passed**(신규 2개 — 목표 17 in-flight 중 마커 10 거부 / 마커 17 수용).

**수정 파일**: `server/core/_marker_mixin.py`, `virtual_test/algorithm/test_marker_adjacency.py`

---

### 수정 65: 트윈 페이싱 — 빠른 쪽 편향 + 양단 동기화 + 불일치 노출 (2026-07-12)

> **설계 원칙**: 보간하는 이상 추정은 **반드시 틀린다**. 그러면 문제는 "안 틀리는 법"이 아니라 **"어느 방향으로 틀릴 것인가"**다.
>
> | 실패 모양 | 사람 눈에 | 정보 |
> |---|---|---|
> | 너무 빠름 → 먼저 도착해 **기다림** | "잠깐 멈췄네" (자연스러움) | **대기 시간 = 오차 크기** (보존됨) |
> | 너무 느림 → **순간이동**으로 끌려감 | "고장났네" (신뢰 붕괴) | 언제 얼마나 틀렸는지 **소실** |
>
> 무게가 전혀 다르다. → **일부러 빠르게 편향해 항상 조금 먼저 도착하게 한다.**

- **(1) 속도 편향** (`TWIN_SPEED_BIAS`, 기본 0.85): 실측 평균을 그대로 쓰지 않고 85%로 짧게 잡아 페이싱 → 항상 일찍 닿는다. `TWIN_HOLD_RATIO` 기본 0.99 → **1.0**(노드까지 완전히 가서 대기).
- **(2) 양단 동기화** — 역할이 다르다:
  - **출발 시**(`_start_twin_pacing`) = **원점 리셋**. 보간이 시작되는 위치를 노드에 정확히 박아 **오차가 다음 엣지로 누적되지 않게** 한다. 정상 상황엔 이미 노드에 있으므로 공짜(no-op).
  - **도착 시**(`sync_to_node`) = 최종 위치 확정 (기존).
- **(3) ★불일치를 숨기지 말고 드러낸다**: 일찍 도착하면 **실물이 중간에 끼어 멈춰도 트윈은 목적지에 서 있어 멀쩡해 보인다** — 매끄럽게 보이려다 트윈의 본질(정직함)을 잃는 함정. 실제로 오늘 트윈이 서버의 heading 오류를 신고해 수정 62/64를 발견하게 했다. → 도착 시 **스냅 거리**(>0.30m 경고 = 추정이 느렸거나 서버가 현실과 어긋남)와 **대기 시간**(>5s 경고 = 실물이 끼었을 수 있음)을 로그로 노출.
- **미적용(다음 단계) — 회전 실시간 추종**: AGV는 노드 위에서 제자리 회전하므로 **발밑 마커가 계속 보인다** → 회전은 추정이 아니라 **카메라 yaw로 측정** 가능하다. (직진은 마커가 시야를 벗어나 보간이 불가피 — **이 비대칭이 설계의 핵심**.) 카메라가 이미 매 프레임 `(x, y, yaw)`를 계산해 **STM으로만** 보내고 있으므로 `/agv/pose`로 한 줄 더 발행하면 된다. **엔코더도 펌웨어 수정도 불필요** — 수정 60의 "STM 프로토콜 확장 필요"라는 한계 기술은 이 점에서 **틀렸다**.
  - 선결: `camera.py`의 `[TODO 미팅] yaw_deg → 서버 heading(0=N/90=E) 변환 규약` 실측 확정.
  - 전제 확인 필요: 카메라가 **회전축 근처**에 있어야 회전 중 마커가 시야에 남는다.

**수정 파일**: `isaac_simulation/step7_kinematic.py`

---

### 수정 66: 트윈에도 서버와 같은 마커 가드 (2026-07-12)

> **쉽게**: **서버는 이상한 마커를 거부하는데 트윈은 그냥 믿었다.**
> ```
> 서버:  불가능한 마커 3 무시 (현재 10, 이웃 [9,11,2,18])
> 트윈:  실물 마커 3 → 위치 동기화          ← 따라가버림
> ```
> 그 순간부터 둘은 **다른 위치를 믿는다.** 트윈은 현실도 아니고 서버 생각도 아닌 아무것도 아닌 걸 그리게 된다. 실물 카메라가 마커를 한 번 잘못 읽으면 바로 이 상태다.

- **가드** (`sync_to_node`): 수정 62/64와 **같은 논리**를 트윈에도. forward 중이면 목표 노드만, 정지 중이면 현재 노드 또는 이웃만 수용. "로봇은 한 칸씩 굴러가지 순간이동하지 않는다"는 서버만의 상식이 아니라 **물리 법칙**이다.
- **예외 — 첫 마커는 무조건 수용**(`_twin_localized`): 트윈을 서버 도중에 켜면 자기 위치를 모른다. 서버엔 이 예외가 없다(AGV가 홈에서 시작한다고 전제). **트윈은 아무것도 제어하지 않으므로** 첫 보고를 믿어도 위험하지 않다.
- **덤**: 스냅 경고에 원인 구분 추가 — `forward 명령 없이 마커만 들어옴`(페이싱할 기회가 없었음) vs `추정이 느렸음`. 원인이 다른데 같은 메시지가 나오면 오진한다.

**수정 파일**: `isaac_simulation/step7_kinematic.py`

---

### 수정 67: UART 오프셋 주기를 카메라 프레임률에서 분리 (2026-07-12)

> **쉽게**: `set_marker_offset()`이 **카메라 프레임마다** UART 패킷을 STM에 쐈다. 즉 **UART 전송 주기 = 카메라 프레임률**로 우연히 묶여 있었다. 그래서 카메라를 최적화하면(아래 미적용 항목) **STM이 받는 패킷 수가 조용히 1.7배로 올라간다** — 아무도 의도하지 않았는데. 카메라 성능을 건드릴 때마다 STM 부하를 걱정해야 하는 구조였다.

- **수정**: `config.UART_OFFSET_HZ`(기본 **10** = 현 실제 주기) 상한. 최신 값은 항상 갱신하되 **전송만** 제한 → 오래된 값을 보내는 일은 없다. **명령(turn/lift/forward)은 제한 없음** — 즉시 나가야 한다.
- **검증**: 카메라 30fps로 밀어넣어도 UART는 **10.0Hz**로만 나감. SIL 통과.
- **현 동작 그대로 유지** — 상한을 지금 나오는 속도에 맞췄으므로 STM 입장에선 아무것도 안 바뀐다.
- **대역폭은 원래 문제가 아니었다**: 115200 baud, 21바이트 패킷 → 10Hz면 대역폭의 2.7%. 제약은 대역폭이 아니라 **STM의 처리 여력**이고, 그건 우리가 모른다. → 그래서 **명시적 상수로 만들어 둔다.**

#### 카메라 파이프라인 — 이중 왜곡보정은 ⛔**고치지 말 것** / 프레임 절벽만 별도로

실측 프로파일 (`camera.detect()` = 53ms → 15fps):
```
capture   : 51.2 ms (78%)   ← 프레임 대기 (33.3ms 주기를 놓쳐 두 프레임 기다림)
undistort : 10.0 ms (15%)
detect    :  3.5 ms
solvePnP  :  0.0 ms
```

**현상**: `cv2.undistort(frame)`로 왜곡을 지운 뒤 `solvePnP(..., dist_coeffs)`에 **왜곡계수를 또 넘긴다** → 수학적으로는 과보정. 주원이 원본(`opencv_arucomarker_detection_v4.py:88/113`)에서 유래.

> ⛔ **[절대 금지] 수식을 "올바르게" 고치지 말 것 (2026-07-12 사용자 지시).**
>
> **주원이의 STM이 직진·회전 계산을 지금 나오는 x/y/yaw 값 기준으로 이미 튜닝해놨다.**
> 카메라 수식을 바꾸면 값이 바뀌고 → **그 튜닝이 전부 어긋난다.**
>
> 즉 이건 '고쳐야 할 버그'가 아니라 **시스템이 이미 그 위에서 보정된 상수**다.
> **하류에서 보상된 오차를 상류에서 단독으로 고치면 시스템이 깨진다.**
> (수학적 오차 자체도 작다: z는 38mm까지 틀리나 `detect()`가 반환조차 안 함 /
>  yaw 2.6°는 90° 반올림에 묻힘 / x·y는 정렬되며 중앙으로 갈수록 0으로 수렴)

**프레임 절벽은 별개로 해결 가능** — 값을 바꾸지 않고 속도만 올리는 길이 있다:
- `cv2.undistort()`는 내부적으로 매 프레임 **왜곡 맵을 새로 계산**한다.
  `cv2.initUndistortRectifyMap()`으로 맵을 **한 번만** 만들어두고 `cv2.remap()`만 호출하면
  **출력이 수학적으로 동일**하면서(= 주원이 튜닝 안전) 훨씬 빠르다.
- 처리 14.5ms → 한 프레임(33.3ms) 안에 들어가면 **15fps → ~26fps**.
- 수정 67 덕분에 카메라가 빨라져도 **STM 부하는 안 변한다**(UART 10Hz 고정).

**수정 파일**: `hardware/config.py`, `hardware/bridge_rpi.py`

---

### 수정 68/69: 트윈이 실물 회전을 실시간으로 따라감 + heading 배관 (2026-07-12)

> **쉽게**: 직진과 회전은 사정이 다르다. 직진 중엔 마커가 카메라 밖으로 나가서 **볼 수가 없다**(시간으로 보간할 수밖에 없음). 그런데 **회전 중엔 발밑 마커가 계속 보인다** → 실제 각도를 **측정할 수 있다**. 그동안 트윈은 이 차이를 무시하고 회전도 시간으로 보간해서, 실물과 최대 28° 어긋났다.

- **수정 68**: `/agv/pose` 토픽 신설 (트윈 전용, 20Hz). 실물이 카메라로 읽은 연속 yaw를 흘려보내고 트윈이 **그대로 따라 돈다**. 회전 시작마다 기준(`_pose_ref`)을 **다시 잡는다** — 이걸 안 해서 오차가 누적됐다.
- **수정 69**: 서버가 카메라 heading을 **받아서 비교하고 로그만 찍는다**(`TRUST_CAMERA_HEADING=False`). 배관만 깔아두고 **밸브는 잠가둔 상태** — 실물에서 로그를 읽어보고 믿을 만하면 켠다.
- **검증**: yaw 88° → heading 2° 변환까지 1:1 추종 확인.
- **수정 파일**: `isaac_simulation/step7_kinematic.py`, `hardware/bridge_rpi.py`, `hardware/config.py`, `server/core/_marker_mixin.py`, `server/config.py`

---

### 수정 70: forward 명령에 "어디로 가라"를 명시 (2026-07-12)

> **쉽게**: 서버가 `forward`만 보내면, AGV는 **자기가 어느 노드로 가는 중인지 스스로 추측**해야 했다(`_find_forward_target`). 서버는 이미 알고 있는데 안 알려준 것. 추측이 틀어지면 엉뚱한 칸에 도착한다.

- **수정**: `/agv/cmd`에 `target_node` 필드 추가. AGV는 받은 값을 쓰고, 없으면(구버전) 기존 추측으로 폴백.
- **효과**: 수정 64(마커가 forward 목표와 다르면 무시)의 판단 근거가 **서버 의도 그대로**가 된다.
- **수정 파일**: `server/comm/mqtt_client.py`, `server/core/_movement_mixin.py`, `isaac_simulation/bridge_isaac.py`, `isaac_simulation/step6_visual.py`, `isaac_simulation/step7_kinematic.py`, `hardware/bridge_rpi.py`

---

### 수정 71/72/73: 트윈 부트스트랩·페이싱·동적 렌더링 (2026-07-12)

- **수정 71 — 부트스트랩 예외 제거**: "첫 마커는 무조건 믿는다"는 예외 때문에, 시작하자마자 들어온 **유령 마커 37번에 트윈이 5m 순간이동**했다. 예외를 없애고 시작 노드는 `TWIN_START_NODE`로 **명시**한다.
- **수정 72 — 사람 반응시간을 로봇 속도로 학습하던 문제**: 트윈이 한 칸 이동 시간을 9~12초로 배웠다. 벤치에서 **사람이 손으로 마커를 대주는 시간**이 섞인 것. 이상치 필터가 120초라 다 통과했다 → `TWIN_EDGE_MAX=8.0`.
- **수정 73 — 연결된 AGV만 그린다**: 실물이 1대만 켜져 있으면 트윈도 1대만. 2번째가 **도중에 접속하면 그때 나타난다** (`set_agv_visible`).
- **수정 파일**: `isaac_simulation/step7_kinematic.py`

---

### 수정 74: 죽은 예약이 A*를 막던 문제 — 청소부 복원 (2026-07-13)

> **쉽게**: AGV가 30분 전에 지나간 길을 서버가 아직 "누가 지나갈 예정"이라고 믿고 피해 다녔다. 그래서 멀쩡한 지름길을 두고 +2칸씩 돌아갔다 (실측 9/285건).

**근본 원인 — 수정 55가 청소부를 지웠다**

수정 55(통행권 모델)는 "경로를 짤 때마다 **다른 모든 로봇의 예약을 지우고 남은 경로로 다시 박는**" 블록을 `soft_avoid`로 교체했다. 그런데 그 블록은 사실 두 가지 일을 하고 있었다.

1. **청소부** — 지나간 칸 / 죽은 경로 반납
2. **시계 맞추기** — 모든 로봇의 예약 시간축을 "지금"(t=0)으로 재정렬

블록은 지웠는데 **예약을 만드는 쪽(`commit`)은 남겨뒀다.** → 예약은 계속 쌓이는데 아무도 안 지움.

**죽은 예약이 생기던 3가지 경로**

| 경로 | 조치 |
|---|---|
| 로봇이 IDLE로 끝남 → 다시는 계획 안 함 → 마지막 경로가 **영원히** 남음 | `RobotManager.on_idle` 콜백 → 예약 반납 |
| 경로를 완주했지만 IDLE은 아님 (작업대에서 **사람을 기다리는 중**) | 마커 도착 시 `planned_path`가 비면 → 반납 |
| 이동 중 **이미 지나온 칸** (`advance()`가 어디서도 호출 안 됨) | 마커 도착 시 남은 경로를 **t=0부터 재예약** |

- IDLE 전이를 **`set_robot_status` 한 곳으로 통일** (`complete_task`가 `robot.status`에 직접 대입하던 것 제거). 앞으로 IDLE 전이를 추가해도 청소가 자동으로 따라온다 — 수정 55가 빠진 함정을 구조적으로 막음.

**⛔ `commit()` 호출을 지우면 안 된다 (실패 기록)**

죽은 예약이 미우니 `_movement_mixin.py`의 `reservation.commit()`을 아예 지워봤다. → **53건 만에 AGV 2대가 41↔42에서 정면으로 맞물려 완전히 멈췄다.**

`commit()`이 박는 건 cell만이 아니라 **edge(스왑 예약)** 이고, 그게 *"쟤가 B→A로 갈 예정이면 나는 A→B 못 감"* = **정면충돌을 계획 단계에서 막는 유일한 장치**였다. 41번은 이웃이 33·42뿐이라 한번 맞물리면 교착 해소(수정 54)도 빠져나갈 길을 못 찾는다.

→ **예약은 없애는 게 아니라 청소하는 것이다.**

**검증** (랜덤 부하: 주문 38건 / 태스크 154개 / 경로계획 374건)

```
죽은 예약        0건   (수정 전 9/285 = 3.2%)
교착             6건   전부 자연 해소, 정지 0건
No path found    0건
에러             0건
회귀 테스트      93/93 통과
```

우회 54건은 전부 설명됨 — 정당 43건(선반이 놓인 칸 / 다른 AGV가 실제로 서 있는 칸) + 살아있는 예약 11건(그 로봇이 진짜 1~2걸음 뒤에 지나갈 칸).

- **진단 계측 상시화**: `_diagnose_suboptimal_path()` — 고른 경로가 최단이 아닐 때만 **왜 막혔는지**(선반/AGV위치/예약, 예약이면 누가 언제) 찍는다. 최단이면 조용하다. 추측 대신 현장에서 잡기 위한 장치.
- **수정 파일**: `server/managers/robot.py`, `server/core/request_handler.py`, `server/core/_marker_mixin.py`, `server/core/_movement_mixin.py`
...
### 수정 76: 노드/마커 번호를 0-based로 (1~48 → 0~47) + 하드코딩 JSON화 (2026-07-14)

> **쉽게**: **실물 ArUco 마커가 이미 0~47로 인쇄돼 있다.** 소프트웨어(1~48)를 거기에 맞춘 것이다.
> 맵의 모양은 그대로고 **번호만 −1.** (마커를 다시 뽑는 게 아니라, 뽑힌 마커에 코드를 맞췄다.)
> 마커만 0-based로 하고 노드를 1-based로 두는 선택지도 있었지만, 그러면 `마커 = 노드 − 1`이 되어
> 서버·트윈·벤치 곳곳에 +1 암산이 흩어진다. 한 군데만 빠뜨려도 로봇이 **정확히 한 칸 옆에 있다고
> 착각**하는데, 증상이 "유령 마커/순간이동"과 똑같아서 구분이 안 된다. 그래서 **노드까지 같이 내려
> `마커 ID == 노드 ID` 항등을 지켰다.** 실물 붙이는 날 로그에 `marker 17`이 찍히면 그게 곧 노드 17이다.

**바뀐 번호** (맵 좌표·연결·엣지 164개는 불변, 번호만 −1)

| | 옛 | 지금 |
|---|---|---|
| W1 | 33 (gw25/stg41/trg34) | **32** (gw24/stg40/trg33) |
| W2 | 9 (gw17/stg1/trg10) | **8** (gw16/stg**0**/trg9) |
| 선반 1-1~1-4 / 2-1~2-4 | 19,20,27,28 / 22,23,30,31 | **18,19,26,27 / 21,22,29,30** |
| AGV 홈 (1 / 2) | 9 / 33 | **8 / 32** |
| 입고 스테이션 | 48 | **47** |

**핵심 함정 — 노드 0은 falsy다**

W2의 staging_node가 `0`이 되면서, `staging.py`의
`if gateway and staging and trigger:` 한 줄이 **W2 회랑을 통째로 등록하지 않았다**(0이 거짓이라).
스테이징이 절반 죽는데 에러는 안 난다. 회귀 테스트가 잡았다.
→ 노드를 truthiness로 판단하던 곳 5개를 전부 `is None`으로 교체
   (`staging.should_stage` 회랑 등록 / `_trigger_to_ws` 조회 2곳 / `home_node or shelf_id` 2곳 / 벤치 `--auto-walk 0`).
**앞으로 노드 값에 `if not node:` 쓰지 말 것.**

**하드코딩 제거 (번호를 JSON 한 곳으로)** — 번호를 바꾸려면 코드를 뒤져야 했던 것들:

| 위치 | 있던 것 | 지금 |
|---|---|---|
| `db_loader.py` | `SHELF_NODE_MAP` (JSON에 같은 표가 또 있었음) + `33 if user==1 else 9` | `shelf_config.json`에서 로드 |
| `order_optimizer.py` | `NODE_COORDS = {(id-1)%8 ...}` 격자 역산, `SHELF_NODES`(죽은 코드), `start_node=33` | `map.json` 좌표 직접 로드 / 삭제 / 호출자 필수 인자 |
| `robot.py` | 설정 로드 실패 시 홈 `33/34` 폴백 (**실제 홈은 9/33 — 이미 틀린 값**) | 폴백 제거, 즉시 `RuntimeError` (홈을 추측하지 않는다) |
| `Presentation_manual.py` | 자체 맵 (`row*8+col+1`, 선반/작업대/홈 하드코딩) | `server/data/*.json`에서 로드 (서버 맵과 일치 검증됨) |

이제 **맵을 또 바꾸려면 `server/data/*.json`만 고치면 된다.**

**검증**: pytest 100 passed (테스트 14파일의 노드 리터럴도 −1) / 서버 실기동 — 회랑 [8,32] 등록,
주문 1건이 엑셀 → 선반 26·18·30 → A* → `forward(target_node=24)` 까지 발행 확인.

**실물 영향**: **없다 — 실물 마커가 이미 0~47이라 이 변경의 출발점이었다.** 바닥에 그대로 깔면 된다.
시뮬 쪽만 `aruco_0.png`를 신규 생성했다(512×512, 마커 400px + 여백 56px — 기존 규격 동일, ID 0 검출 확인).
`make_marker_sheet` 기본값도 0~47로 맞춰뒀다(추가 인쇄나 낱장 재출력이 필요할 때만 사용).
**협업자 GUI는 무관** — GUI는 `사용자ID/주문번호/작업대/선반번호("1-1")`만 쓰고 노드 번호를 안 쓴다.

⚠️ **JSON을 바꾼 뒤엔 Isaac Sim과 서버를 재시작할 것.** Isaac은 뜰 때 map.json을 한 번만 읽으므로,
안 끄면 서버(0-based)와 Isaac(옛 맵)이 서로 다른 맵을 보고 마커가 전부 어긋난다.

**아래 수정 이력(수정 1~75)의 노드 번호는 옛 1-based다. 현재 번호 = 옛 번호 − 1.**

---

### 수정 79: 선반 든 채 회전 금지 — 회전/리프트 순서 재배치 (2026-07-15, 설계)

> **쉽게**: 선반을 든 채로 제자리 회전하면 **옆 칸 선반과 부딪힌다**(선반이 AGV보다 넓게 돈다).
> 그래서 규칙을 하나로 못박는다 — **회전은 언제나 선반을 안 든 상태에서 한다.** 리프트와 회전의
> 순서를 이 규칙에 맞춰 전부 뒤집는다.

**핵심 불변식: 회전(turn) 시점에 `carrying_shelf`가 없어야 한다.**

**바뀐 시퀀스 (4곳)**

| 시점 | 옛 (든 채 회전 💥) | 새 (안 든 채 회전 ✅) |
|---|---|---|
| **픽업**(선반 노드) | `lift_up` → [이동계획의 turn] → forward | **turn(출발방향)** → `lift_up` → forward |
| **WS 도착** | (든 채 대기) + 피킹방향 회전(수정 58) | **`lift_down`**(내려놓고 대기) — 방향맞춤 제거 |
| **반납/포워딩 출발** | [이동계획의 turn, 든 채] → forward | **turn(경로방향)** → `lift_up` → forward |
| **스테이징 대기** | 회랑 진입 전 미리 회전 | **회전 보류** → 다른 AGV WS 이탈(TRG) 확인 후 회전 |

**설계 결정 (사용자 확인 2026-07-15)**
- **WS에선 선반을 안 들고 대기한다.** 도착 즉시 `lift_down` → 작업자는 바닥의 선반에서 픽킹 →
  `shelf_complete` 시 **처음으로** `lift_up`(반납 출발). WS 체류 내내 리프트는 내려가 있다.
- **수정 58(WS 피킹방향 회전)은 완전 제거.** 선반이 바닥에 있으면 AGV만 돌아 선반은 안 돌아가
  방향맞춤이 물리적으로 불가능하다. `pick_heading`/`_orient_for_picking`/`_ws_orienting` 삭제.
  → 선반은 **들려온 방향 그대로** 바닥에 놓인다(작업자가 어느 면이든 집는다).
- **핸들링 범위**: "선반 노드에서의 첫 회전"이 옆 선반 충돌의 원인이다. 복도 중간 회전은 대상 밖.
  (픽업/반납 모두 **선반 격자를 벗어나기 전 첫 방향전환**을 리프트 앞으로 옮기는 것이 요지.)

**구현 노트 (핵심 파일)**
- 픽업: `_process_arrival`(GO_TO_SHELF→PICKUP)에서 `lift_up` 즉시 발행 대신, **WS 이동을 먼저 계획**해
  선두 turn을 뽑아 `[turn…, lift_up, forward…]` 큐 구성. `carrying_shelf`/`mark_shelf_picked_up`는
  **`lift_up` ack 시점으로** 이동(회전 중엔 아직 안 든 상태여야 하므로).
- WS 도착: `_process_arrival`(DELIVER_TO_WS)에서 `_enter_wait_picking` 대신 `lift_down` 발행 →
  putdown ack에서 WAITING_FOR_PICK 전이 + `shelf/arrived` 통지. `_orient_for_picking` 경로 삭제.
- 반납 출발: `_handle_shelf_complete`/`_handle_pickup_ack`(포워딩)의 `_plan_and_publish_move` 결과 큐를
  `[turn…, lift_up, forward…]`로 감싼다(선반은 WS 바닥 → 회전 후 `lift_up`).
- 스테이징(2-1): **코드 변경 불필요 — 기존 구조가 이미 만족.** 스테이징 노드 도착은
  `_process_arrival`(lift_down)이 아니라 `is_staged_agv → "holding commands"`로 빠져 lift·회전
  모두 없음. 선반은 격자에서 든 채 스테이징 통과(carrying 유지, lift_down 없음 = "들었다놨다" 없음),
  회전은 해제 시 `_dispatch_released_agv`/`handle_marker_trigger`의 WS-진입 이동에서만 발생.
  구동 검증: 배달→스테이징(0) 우회 시 carrying 유지·회전 없이 HOLD → 해제 후 `turn_left forward lift_down`.

**리플/후속 (별도)**
- 트윈(`step7_kinematic.py`): lift/turn 순서·WS lift_down 대기를 그대로 반영해야 함.
- 주원 펌웨어: 이산 명령을 순서대로 실행하므로 **순서 변경은 그대로 동작**. 다만 WS에서 lift_down+
  나중 lift_up 사이클이 물리적으로 추가됨(선반이 바닥에 닿음) — HIL에서 확인.
- pytest: `test_lift_shelf_id`/`test_command_queue`/`test_stg` + 수정 58 orient 가정 갱신 필요.

**검증**: ✅ **서버 구현 완료 (2026-07-16).** pytest 100 passed / 퍼저 3종(경로·예약·교착) 무회귀 /
in-process 전체 구동 검증: 픽업 `[turn, lift_up, forward…]` · WS 도착 `lift_down`→WAIT · 반납 출발
`[turn, lift_up, forward…]` · 스테이징 carrying 유지+회전 해제시. **남은 것: 트윈(step7)·주원 펌웨어·시뮬 4시나리오.**
`shelf_config.json`의 `pick_heading` 필드는 이제 미사용(제거해도 무방, 데이터라 방치는 무해).

**시뮬 실기동 확인 (step6, 2026-07-16)**: 서버+step6+GUI 구동 시 픽업 `turn→lift_up`, WS `lift_down→wait_picking`
로그로 확인됨(수정 79 정상). step6/step7 모두 코드 변경 불필요(`_place_shelf`가 shelf_origins 갱신 →
WS에 놨다 재픽업 자동 처리). ⚠️운영 함정: **서버·Isaac·주문서버는 한 벌만** — 두 벌 켜면 발행자 2명 →
포트 5000 충돌 + 선반상태 split + presence flapping으로 태스크가 꼬임(서버 재시작 필요).

---

### 수정 80: 든 선반 회전-간섭 — 라우팅으로 원천 봉쇄 (2026-07-16)

> **문제(사용자 관찰, step6)**: 선반을 **든 채 제자리 회전**하면 든 선반이 사방으로 부풀어(s/2→s/√2)
> 직교(상하좌우) 인접칸을 침범 → 옆의 **정적 선반** 또는 **다른 든 로봇의 선반**과 닿는다. (대각선은 안전.)
> 현재 예약 모델은 "같은 칸+스왑"만 막고 직교 인접은 허용하므로 이 충돌을 못 막는다.

**핵심 통찰**: 위험한 건 "지나가기"가 아니라 **"도는 것"**이다. 든 로봇이 직진으로 통과하면 선반은
자기 칸 안에 있어 안전. 회전할 때만 부푼다. 그리고 반응형(회전 보류)이 교착이 되는 건 **안 움직이는 것
(정적 선반)을 기다릴 때뿐** — 상대가 움직이는 로봇이면 곧 비키므로 미뤄도 안전하다. → 두 위험을 분리.

**① 정적 선반 옆 회전 → 라우팅 배제 (채택, 구현 완료)**
- 실측(A* 8선반×2작업대×4방향=64조합): **위험한 든-채-회전 12건이 전부 노드 20·28에서만** 발생.
  17·23·25·31은 0건 — 그 노드들의 유일한 선반 이웃이 "지금 든 그 선반"이라 이미 빈칸이기 때문.
  20·28만 옆이 서로 다른 두 안쪽 선반(19·21 / 27·29)이라 하나를 들어도 나머지가 선다.
- **해결**: 선반 든 상태(`carrying_shelf is not None`)에선 20·28을 IN_PLACE 선반 노드처럼 `excluded_transit`에
  추가 → A*가 절대 그 위에서 돌지 않음. **재검증: 위험회전 0건, 경로없음 0건(64조합 다 성립).**
- ⚠️ 숫자 하드코딩 금지(규칙 11): 20·28을 코드에 박지 않고 `PathPlanner`가 맵에서 유도한다 —
  "비선반 노드인데 직교 이웃에 선반이 ≥2개" = `carry_forbidden_nodes`. 맵이 바뀌어도 자동으로 맞음.
- 구현: `path_planner._load_map`(집합 계산) + `_movement_mixin._plan_and_publish_move`(든 상태면 배제 합집합).
- "내려놓고 돌기"(선반 상태 CARRIED↔바닥 전환 = 규칙 3 위험구역) **불필요** — 라우팅만으로 끝. 큰 장점.

**② 다른 든 로봇 옆 회전 → 스테이징 라우팅으로 마주보기 방지 (기존 메커니즘 활용)**
- 두 든 로봇이 직교 인접+회전하려면 보통 **같은 작업대로 수렴**할 때. 기존 staging이 이미 회랑을 직렬화
  (`should_stage`가 2번째 로봇을 staging_node=40/0 = **스테이징 행**으로 보냄) + 홈 반대배치 + 교착감지기(수정 54).
- 방침: 반응형 회전-보류(교착 위험) 대신 **수렴 시 스테이징 행 경유를 보장**해 마주보기 자체를 안 만든다.
  → 대부분 이미 성립. 잔여 뚫림 지점만 최소 변경(staging은 cascade race 핫스팟이라 신중).

**상태: ① 구현 완료. ② 기존 staging으로 상당 부분 커버 — 잔여 갭 확인 후 최소 변경.**

---

### 수정 84: 든로봇 회전 위험 — 내려놓기 전 '대기'(움직이면 곧 비킴) (2026-07-17)

> 수정 82 안전망은 위험이면 무조건 '내려놓고 돌기'. 하지만 위험 원인이 **정적선반**(안 비킴)이면
> 내려놓기가 필수지만, **이동 중인 든로봇**이면 곧 비키므로 내려놓지 말고 **잠깐 대기**가 낫다.

- `_turn_hazard_kind`가 위험 종류 반환("정적선반"/"든로봇"/"정적선반+든로봇"). 순수 든로봇 위험이면
  turn을 보류(defer, `return False`) → `_try_dispatch_all`이 상대 이탈 시 재시도 → 대기가 풀림.
- **대칭교착 방지**: `TURN_DEFER_MAX`(기본 3)회 초과하면 폴백=내려놓고 돌기(양쪽이 내려놓으면 위험 소멸).
- **hang 방지 핵심**: 대기는 **상대가 실제 이동 중(in_flight forward로 이웃 진입)**일 때만
  (`_intransit_carrier_adjacent`). 정지한 든로봇 옆은 안 비키고 재시도 트리거(마커)도 없어 →
  대기 금지, 즉시 내려놓기. **끄기: `TURN_DEFER_MAX=0`**.
- 검증: 라이브 sim 18주문에서 defer 11회 발동 **전부 폴백 없이 자연 해소**(상대 비켜서 진행),
  충돌·행·대칭교착 0. pytest 100.

---

### 수정 85: 든 로봇 회전 충돌 — 예약 기반 근본해결 (2026-07-17)

> **문제(라이브 sim + ORACLE=1로 발견, 재현 3/3)**: 수정 82 안전망(내려놓고 돌기)이 **켜져 있는데도**
> 든 로봇 회전 충돌이 발생. 오라클(실좌표 SAT) 3건 전부 동일 결정론적 패턴 —
> **AGV-1이 노드 17에서 든 채 회전 ↔ AGV-2가 17의 이웃 노드(16/25)로 *진입 중*.** 안전망은 0회 발동.

**근본 원인 = 안전망 든로봇 판정의 in-transit 사각지대.** `_turn_hazard_kind`가 다른 로봇의
**정지 위치(`current_node`)만** 봤다. 상대가 이웃칸으로 이동 중이면 서버상 `current_node`는 아직
이웃이 아니라(마커 도착 전) 위험을 놓친다. 게다가 비대칭: forward 발행 게이트도 "이웃에서 회전 중인
로봇"을 안 봐서, 반대 순서(회전 먼저 커밋 → 그 회전 중 상대가 진입)도 뚫린다.

**해결 = 회전을 "이웃 4칸의 일시 점유"로 예약 모델에 표현** (물리 사실: 회전하는 선반이 대각으로
부풀어 직교 이웃을 회전하는 동안 침범). 예약 단일 진실(`ReservationService`)에 1급으로 추가.

- **순서1 (내가 회전, 상대가 진입 중)**: `_turn_hazard_kind`가 이웃 검사에 상대의 **진입 예정 노드**
  (`command_queue.peek_expected_node()`)도 포함 → 든로봇 위험 감지 → 내려놓고 돌기.
- **순서2 (내가 회전 커밋, 상대가 그 중 진입)**: 든 채 회전 dispatch 시 이웃 4칸을
  **turn footprint 예약**(`reserve_turn_footprint`) → 상대 forward가 그 칸 진입 시
  `turn_lock_owner`로 **대기**. 회전 ack(`_marker_mixin`) 시 `release_turn_footprint`로 해제.
- **교착 없음**: footprint는 in-flight 회전 중에만 잡히고 회전은 곧 ack(상대와 무관하게 완료)되므로
  스스로 풀린다. 상대는 `_try_dispatch_all` 재시도로 자동 해소. presence offline 시에도 해제(누수 방지).

**검증 (동일 6주문 시나리오, ORACLE=1)**: 오라클 실충돌 **3→0**. 든로봇 위험 3회 + footprint 차단 2회
(충돌났던 노드 17↔25를 실제로 막음) + 정적선반 안전망 1회. 교착·에러·shelf None **0**, 전 주문 완주.
pytest 100 통과. **구현: `reservation_service.py`(footprint 예약/해제/조회) + `_movement_mixin`
(_turn_hazard_kind 진입노드 포함 + dispatch 시 footprint 예약 + forward 게이트 차단) +
`_marker_mixin`(turn ack/offline 해제).**
...
(A/B로 확인 — 85 없는 구코드에서도 재현). 정상~중부하(≤18주문)에선 안 나거나 자기해소됨.

- **[②] shelf-None 포워딩 재리프트 (시뮬 전용)**: 포워딩으로 WS에서 선반을 다시 들 때 Isaac의
  proximity 게이트(`_shelf_is_near`, tol 0.15셀)가 서버 지정 선반을 거부 → `carrying_shelf=None`,
  **빈손 주행 + 선반 유기** → 유기된 선반에 2차 충돌 캐스케이드. **드문 타이밍 레이스**(24주문 5회 /
  다른 24주문 0회). **실물엔 없음**(물리 리프트는 밑의 선반을 그냥 듦). 후보 픽스=Isaac이 서버 지정
  `target_shelf`를 신뢰(게이트 완화). 미구현.
- **[③] 회랑 정면 교착 (실물에도 존재)**: 왼쪽 열(0-8-16-24-32-40 = W2·게이트·W1·스테이징이
  다 매달린 1차선 척추)에서 두 로봇이 반대 방향 종단 중 게이트/구석에서 스왑 교착. backstop(수정 54)은
