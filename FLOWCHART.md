...
#### [미적용] 카메라 파이프라인 — 이중 왜곡보정 + 프레임 절벽

실측 프로파일 (`camera.detect()` = 53ms → 15fps):
```
capture   : 51.2 ms (78%)   ← 프레임 대기
undistort : 10.0 ms (15%)
detect    :  3.5 ms
solvePnP  :  0.0 ms
```
- **버그**: `cv2.undistort(frame)`로 왜곡을 지운 뒤, `solvePnP(..., dist_coeffs)`에 **왜곡계수를 또 넘긴다** → 과보정. (주원이 원본 `opencv_arucomarker_detection_v4.py`에서 유래. 왜곡계수 k1=-0.45로 큼)
- **실제 피해는 작다**(과장 금지): z는 최대 38mm 틀리나 **안 쓴다**(`detect()`는 id/x/y/yaw만 반환). yaw는 최대 2.6° → **90° 반올림에 묻힌다**. x/y는 정렬될수록 중앙으로 가며 **오차가 0으로 수렴**한다.
- **고칠 값어치는 정확도가 아니라 속도**: `undistort` 제거 시 처리 14.5ms → 4.5ms. 카메라 프레임 주기가 33.3ms(30fps)라 **지금은 한 프레임 안에 못 들어가 두 프레임을 기다린다**(→15fps). 10ms만 줄이면 절벽을 넘어 **~26fps**.
- **수정 67을 먼저 넣었으므로** 이 최적화를 해도 STM 부하는 안 변한다.
