...
Window.size = (800, 480)
# ✅ Window 배경색을 갈색으로 설정
Window.clearcolor = (0.894, 0.820, 0.482, 1)
...
class WorkCell(Button):
    ...
    def __init__(self, **kwargs):
        ...
        self.background_color = (0.820, 0.757, 0.471, 1)  
        self.color = (1,1,1,1)  # 검은색 텍스트
        self.font_size = '18sp'
    ...
    def on_press(self):
        ...
        if not self.is_completed:
            self.background_color = (0.75, 0.5, 0, 0.75)  # 빨간색
...
class WarehouseGUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            Color(0.894, 0.820, 0.482, 1)  # 갈색 (R, G, B, A)
    ...
    def build_top_section(self):
        ...
        for manager_id in sorted(LOGISTICS_DATA.keys()):
            btn = Button(
                ...
                font_size='20sp',
                font_name=FONT_NAME,
                background_color=(0.820, 0.757, 0.471, 1), # 파란색 계열
        ...
        self.work_list_btn = Button(
            ...
            font_size='20sp',
            font_name=FONT_NAME,
            background_color=(0.820, 0.757, 0.471, 1), # 연한 회색
        ...
        self.start_work_btn = Button(
            ...
            font_size='20sp',
            font_name=FONT_NAME,
            background_color=(0.820, 0.757, 0.471, 1), # 녹색
            color=(1,1,1,1), # 글자색 흰색
            disabled=True
        )
    ...
    def build_bottom_section(self):
        ...
        self.complete_btn = Button(
            ...
            font_size='24sp',
            font_name=FONT_NAME,
            background_color=(0.820, 0.757, 0.471, 1), # 파란색
            color=(1,1,1,1), # 글자색 흰색
            disabled=True
        )
    ...
    def on_manager_select(self, instance):
        ...
        self.work_list_btn.background_color = (0.820, 0.757, 0.471, 1) # 활성화 시 연한 회색
        self.start_work_btn.disabled = False
        self.start_work_btn.background_color = (0.820, 0.757, 0.471, 1) # 활성화 시 녹색
        
        # 그리드 초기화
        for cell in self.work_cells:
            cell.text = ''
            cell.background_color = (0.820, 0.757, 0.471, 1) # 연한 회색으로 초기화
        ...
        self.complete_btn.background_color = (0.820, 0.757, 0.471, 1) # 비활성화 시 어두운 회색 (변경)
    ...
    def show_work_list(self, instance):
        ...
        if self.selected_manager and self.selected_manager in WORK_LIST:
            ...
            with content.canvas.before:
                Color(0.894, 0.820, 0.482, 1)  # 갈색 배경
            ...
            close_btn = Button(
                ...
                font_size='18sp',
                font_name=FONT_NAME,
                background_color=(0.820, 0.757, 0.471, 1), # 닫기 버튼 색상 변경
                color=(1,1,1,1) # 글자색 검정
            )
    ...
    def start_work(self, instance):
        ...
        if self.selected_manager and self.selected_manager in LOGISTICS_DATA:
            ...
            self.complete_btn.background_color = (0.820, 0.757, 0.471, 1) # 활성화 시 밝은 파란색
            ...
            if self.current_task_index < len(tasks):
                ...
                for i, cell in enumerate(self.work_cells):
                    if i < len(current_task):
                        ...
                        cell.text = ''
                        cell.background_color = (0.820, 0.757, 0.471, 1) # 빈 셀은 더 연한 회색으로
            else:
                # 모든 작업 완료
    ...
    def blink_cells(self, cells, count):
        ...
        def toggle_color(dt):
            if blink_state['count'] >= blink_state['max_count']:
                ...
                for cell in cells:
                    cell.background_color = (0.820, 0.757, 0.471, 1) 
                return False  # 스케줄 중단
            
            # 노란색 <-> 연한 회색 토글
            if blink_state['is_yellow']:
                for cell in cells:
                    cell.background_color = (0.820, 0.757, 0.471, 1)  # 연한 회색
                blink_state['is_yellow'] = False
            else:
                for cell in cells:
                    cell.background_color = (0.88, 0, 0, 0.71)  # 노란색
    ...
    def show_completion_message(self):
        ...
        close_btn = Button(
            ...
            font_size='18sp',
            font_name=FONT_NAME,
            background_color=(0.820, 0.757, 0.471, 1), # 확인 버튼 색상 변경
            color=(1,1,1,1) # 글자색 검정
        )
        content.add_widget(close_btn)
        ...
        popup = Popup(
            ...
            content=content,
            size_hint=(0.6, 0.4),
            title_color=(1,1,1,1), # 팝업 타이틀 글자색 검정
        )
        close_btn.bind(on_press=popup.dismiss)
        popup.open()
